...
def _calculate_num_points_in_gt(data_path, infos, relative_path, remove_outside=True, num_features=4):
    for info in infos:
        ...
        P2 = info['calib/P2']
        if remove_outside:
            points_v = box_operations.remove_outside_points(points_v, rect, Trv2c, P2,
                                                        info["img_shape"])
        ...
        gt_boxes_lidar = box_operations.box_camera_to_lidar(
            gt_boxes_camera, rect, Trv2c)
        indices = box_operations.points_in_rbbox(points_v[:, :3], gt_boxes_lidar)
...
def create_groundtruth_database(data_path,
    ...
    for info in prog_bar(kitti_infos):
        ...
        Trv2c = info['calib/Tr_velo_to_cam']
        if not lidar_only:
            points = box_operations.remove_outside_points(points, rect, Trv2c, P2,
                                                        info["img_shape"])
        ...
        rbbox_lidar = box_operations.box_camera_to_lidar(rbbox_cam, rect, Trv2c)
        ...
        point_indices = box_operations.points_in_rbbox(points, rbbox_lidar)
